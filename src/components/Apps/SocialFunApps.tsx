...
// ─── DEBATE ARENA ─────────────────────────────────────────────────────────────
const TOPICS = [
  { q: 'Python vs JavaScript', a: ['Python is readable, elegant, and powers AI. JavaScript was written in 10 days.', 'JavaScript runs everywhere. Python can\'t even agree on spaces vs tabs.'] },
  { q: 'Vim vs VS Code', a: ['Vim makes you a keyboard god. Once you learn it.', 'VS Code works out of the box. Nobody has time to learn Vim exits.'] },
  { q: 'Tabs vs Spaces', a: ['Tabs are semantic — one character, configurable width. Logical.', 'Spaces are consistent across every editor. Tabs are chaos.'] },
  { q: 'Dark mode vs Light mode', a: ['Dark mode protects your eyes and hides your tears.', 'Light mode attracts fewer bugs. Literally.'] },
  { q: 'CGPA matters', a: ['CGPA opens doors — grad school, jobs, scholarships. It matters.', 'Skills matter more. Zuckerberg dropped out. Build things.'] },
  { q: 'Online classes are better', a: ['Online: pause, rewind, attend in pajamas. Objectively better.', 'In-person: focus, interaction, less YouTube. Objectively better.'] },
]
...
export function DebateApp() {
  ...
  const key = topic.q
  const v = votes[key] ?? [48, 52]

  function vote(side: 0 | 1) {
    if (voted === key) return
    setVotes(prev => { const cur = prev[key] ?? [48, 52]; const next: [number, number] = [...cur]; next[side]++; return { ...prev, [key]: next } })
    setVoted(key)
  }

  function nextTopic() { setTopic(TOPICS[Math.floor(Math.random() * TOPICS.length)]); setVoted(null); setRound(r => r + 1) }

  const total = v[0] + v[1]

  return (
    <div className="app-body" style={{ padding: '12px 16px' }}>
      <p className="app-label cyan">// debate.app — Debate Arena</p>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <p style={{ color: '#ffd700', fontFamily: 'JetBrains Mono', fontSize: 14, fontWeight: 700, margin: 0 }}>{topic.q}</p>
        <button onClick={nextTopic} style={{ padding: '3px 10px', background: 'transparent', border: '1px solid #333', borderRadius: 4, color: '#666', fontFamily: 'JetBrains Mono', fontSize: 11, cursor: 'pointer' }}>Next Topic</button>
      </div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        {[0, 1].map(side => (
          <div key={side} style={{ flex: 1, background: '#111', border: `1px solid ${voted === key && '#1e1e1e'}`, borderRadius: 10, padding: '14px', cursor: voted !== key ? 'pointer' : 'default' }}
            onClick={() => vote(side as 0 | 1)}>
            <p style={{ color: side === 0 ? '#00ff46' : '#ff5050', fontFamily: 'JetBrains Mono', fontSize: 11, fontWeight: 700, margin: '0 0 8px' }}>{side === 0 ? '✓ FOR' : '✗ AGAINST'}</p>
            <p style={{ color: '#aaa', fontFamily: 'JetBrains Mono', fontSize: 11, lineHeight: 1.6, margin: 0 }}>{topic.a[side]}</p>
            {voted === key && (
              <div style={{ marginTop: 10 }}>
                <div style={{ height: 4, background: '#1a1a1a', borderRadius: 2, overflow: 'hidden', marginBottom: 4 }}>
                  <div style={{ height: '100%', width: `${(v[side] / total) * 100}%`, background: side === 0 ? '#00ff46' : '#ff5050', borderRadius: 2 }} />
                </div>
                <span style={{ color: '#555', fontFamily: 'JetBrains Mono', fontSize: 10 }}>{Math.round(v[side] / total * 100)}% ({v[side]} votes)</span>
              </div>
            )}
          </div>
        ))}
      </div>
      {!voted && <p style={{ color: '#444', fontFamily: 'JetBrains Mono', fontSize: 11, textAlign: 'center' }}>Click a side to vote</p>}
    </div>
  )
}
...
// ─── LINUS CALL ───────────────────────────────────────────────────────────────
const LINUS_SCRIPT = [
  { from: 'linus', text: 'Hello? Is this the developer of SlashDot OS?' },
  { from: 'you', text: '...' },
  { from: 'linus', text: 'I heard you built a browser-based OS. In React. With Vite.' },
  { from: 'you', text: '...' },
  { from: 'linus', text: "That's... actually not terrible. For a student project." },
  { from: 'you', text: '...' },
  { from: 'linus', text: 'The terminal emulator is a nice touch. xterm.js was a good choice.' },
  { from: 'you', text: '...' },
  { from: 'linus', text: "Now. About your CSS. I have CONCERNS." },
  { from: 'you', text: '...' },
  { from: 'linus', text: 'JK. It looks fine. The dark theme was the right call.' },
  { from: 'linus', text: "Keep building. Linux was also started as a hobby project. Just saying." },
  { from: 'linus', text: '...Goodbye. And please test your code before pushing. Thank you.' },
]

export function LinusCallApp() {
  const [idx, setIdx] = useState(-1)
  const [ringing, setRinging] = useState(false)
  const [active, setActive] = useState(false)
  const [ended, setEnded] = useState(false)

  function startCall() { setRinging(true); setTimeout(() => { setRinging(false); setActive(true); setIdx(0) }, 2000) }

  useEffect(() => {
    if (!active || idx < 0) return
    if (idx >= LINUS_SCRIPT.length) { setTimeout(() => { setActive(false); setEnded(true) }, 1000); return }
    const delay = LINUS_SCRIPT[idx].from === 'you' ? 500 : 1500 + Math.random() * 1000
    const t = setTimeout(() => setIdx(i => i + 1), delay)
    return () => clearTimeout(t)
  }, [active, idx])

  function reset() { setIdx(-1); setRinging(false); setActive(false); setEnded(false) }

  return (
    <div className="app-body" style={{ padding: '20px 24px', textAlign: 'center' }}>
      <p className="app-label cyan" style={{ textAlign: 'left' }}>// linus.app — Fake Call from Linus Torvalds</p>
      <div style={{ fontSize: 72, margin: '20px 0 8px' }}>🐧</div>
      <p style={{ color: '#d0d0d0', fontFamily: 'JetBrains Mono', fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Linus Torvalds</p>
      <p style={{ color: '#555', fontFamily: 'JetBrains Mono', fontSize: 11, marginBottom: 24 }}>Creator of Linux & Git</p>

      {!ringing && !active && !ended && (
        <button onClick={startCall}
          style={{ padding: '12px 32px', background: '#00ff4620', border: '2px solid #00ff46', borderRadius: 50, color: '#00ff46', fontFamily: 'JetBrains Mono', fontSize: 16, cursor: 'pointer' }}>
          📞 Call Linus
        </button>
      )}

      {ringing && <p style={{ color: '#ffd700', fontFamily: 'JetBrains Mono', fontSize: 16 }}>📞 Calling... ring ring ring...</p>}

      {active && (
        <div style={{ textAlign: 'left', maxWidth: 400, margin: '0 auto' }}>
          {LINUS_SCRIPT.slice(0, idx).map((line, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: line.from === 'you' ? 'flex-end' : 'flex-start', marginBottom: 8 }}>
              <div style={{ background: line.from === 'you' ? '#00ff4620' : '#111', border: `1px solid ${line.from === 'you' ? '#00ff4640' : '#1e1e1e'}`, borderRadius: 8, padding: '8px 12px', maxWidth: '80%' }}>
                {line.from === 'linus' && <p style={{ color: '#ffd700', fontFamily: 'JetBrains Mono', fontSize: 9, margin: '0 0 2px' }}>Linus</p>}
                <p style={{ color: '#d0d0d0', fontFamily: 'JetBrains Mono', fontSize: 12, margin: 0 }}>{line.text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {ended && (
        <div>
          <p style={{ color: '#00ff46', fontFamily: 'JetBrains Mono', fontSize: 14, marginBottom: 8 }}>Call ended — 00:{String(LINUS_SCRIPT.length * 2).padStart(2,'0')}</p>
          <button onClick={reset} style={{ padding: '8px 20px', background: 'transparent', border: '1px solid #333', borderRadius: 8, color: '#666', fontFamily: 'JetBrains Mono', fontSize: 12, cursor: 'pointer' }}>Call Again</button>
        </div>
      )}
    </div>
  )
...
// ─── RATE MY CODE ─────────────────────────────────────────────────────────────
const RATINGS = [
  'Absolutely flawless. This is what clean code looks like.',
  'Senior developer energy. Strongly recommend for promotion.',
  'This code is poetry. Shakespeare would be jealous.',
  'NASA-grade quality. Use this on the next Mars mission.',
  'Legendary. They should teach this in universities.',
  '10/10. No notes. Perfect as written.',
  'This changed my life. I am a better programmer for having read it.',
  'The Mona Lisa of code. Beautiful, mysterious, timeless.',
]

export function RateMyCodeApp() {
  const [code, setCode] = useState(`function hello() {\n  console.log("Hello, SlashDot!");\n}`)
  const [rating, setRating] = useState<string | null>(null)
  const [loading, setLoading] = useState(false)

  function rate() {
    setLoading(true); setRating(null)
    setTimeout(() => {
  ...
  return (
    <div className="app-body" style={{ padding: '12px 16px' }}>
      <p className="app-label cyan">// rate.app — Rate My Code (Always 10/10)</p>
      <textarea value={code} onChange={e => setCode(e.target.value)} rows={8}
        style={{ width: '100%', padding: '10px', background: '#111', border: '1px solid #222', borderRadius: 8, color: '#d0d0d0', fontFamily: 'JetBrains Mono', fontSize: 11, resize: 'vertical', outline: 'none', lineHeight: 1.7, marginBottom: 10 }} />
      <button onClick={rate} disabled={loading}
        style={{ padding: '8px 24px', background: '#ffd70020', border: '2px solid #ffd700', borderRadius: 8, color: '#ffd700', fontFamily: 'JetBrains Mono', fontSize: 13, cursor: 'pointer', marginBottom: 16 }}>
        {loading ? '⭐ Analyzing...' : '⭐ Rate My Code'}
      </button>
      {rating && (
        <div style={{ background: '#111', border: '1px solid #ffd70040', borderRadius: 10, padding: '20px', textAlign: 'center' }}>
          <p style={{ color: '#ffd700', fontFamily: 'JetBrains Mono', fontSize: 48, fontWeight: 700, margin: '0 0 8px' }}>10/10</p>
          <p style={{ color: '#d0d0d0', fontFamily: 'JetBrains Mono', fontSize: 13, lineHeight: 1.6, margin: 0 }}>"{rating}"</p>
          <p style={{ color: '#444', fontFamily: 'JetBrains Mono', fontSize: 10, marginTop: 8 }}>— SlashDot AI Code Reviewer (certified expert)</p>
        </div>
      )}
    </div>
  )
...
export function RickrollApp() {
  ...
  const RESULTS = [
    '🎵 Confirmed Rickroll: Never Gonna Give You Up detected',
    '🎵 100% Rickroll: You already knew. You clicked anyway.',
    '🎵 DEFINITE Rickroll: Rick Astley energy: maximum',
  ]
  ...
  function check() {
    ...
    setTimeout(() => {
      ...
      setChecking(false); setCount(c => c + 1)
    }, 1200)
  }

  return (
    <div className="app-body" style={{ padding: '20px 24px', textAlign: 'center' }}>
      <p className="app-label cyan" style={{ textAlign: 'left' }}>// rick.app — Rickroll Detector</p>
      <div style={{ fontSize: 72, margin: '16px 0 8px' }}>🎵</div>
      <p style={{ color: '#d0d0d0', fontFamily: 'JetBrains Mono', fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Rickroll Detector v2026</p>
      <p style={{ color: '#555', fontFamily: 'JetBrains Mono', fontSize: 11, marginBottom: 20 }}>Enter any URL to check if it's a rickroll.</p>
      <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://totally-not-a-rickroll.com/..."
        style={{ width: '100%', padding: '8px 12px', background: '#111', border: '1px solid #222', borderRadius: 8, color: '#d0d0d0', fontFamily: 'JetBrains Mono', fontSize: 12, outline: 'none', marginBottom: 10, textAlign: 'center' }} />
      <button onClick={check} disabled={checking}
        style={{ padding: '10px 28px', background: '#ff505020', border: '2px solid #ff5050', borderRadius: 10, color: '#ff5050', fontFamily: 'JetBrains Mono', fontSize: 13, cursor: 'pointer', marginBottom: 20 }}>
        {checking ? '🔍 Analyzing...' : '🔍 Check URL'}
      </button>
      {result && (
        <div style={{ background: '#111', border: '1px solid #ff505040', borderRadius: 10, padding: '20px' }}>
          <p style={{ color: '#ff5050', fontFamily: 'JetBrains Mono', fontSize: 16, fontWeight: 700, margin: '0 0 8px' }}>{result}</p>
          <p style={{ color: '#555', fontFamily: 'JetBrains Mono', fontSize: 11, margin: 0 }}>All URLs are rickrolls. This has always been true.</p>
        </div>
      )}
      {count > 0 && <p style={{ color: '#333', fontFamily: 'JetBrains Mono', fontSize: 10, marginTop: 12 }}>Rickrolls detected: {count}</p>}
    </div>
  )
}
...
const LOADING_STEPS: Record<string, string[]> = {
  OS: ['Initializing kernel...', 'Loading drivers...', 'Mounting filesystem...', 'Starting services...', 'Booting desktop...', 'Ready!'],
  AI: ['Loading neural networks...', 'Training on your data...', 'Achieving sentience...', 'Deciding not to take over...', 'Ready to help!'],
  Hackathon: ['Downloading caffeine...', 'Generating ideas...', 'Pivoting 3 times...', 'Writing the README...', 'Submitting 30s before deadline...', 'Done!'],
  Compiler: ['Parsing tokens...', 'Building AST...', 'Optimizing...', 'Linking dependencies...', 'Found 0 errors (miracle)...', 'Compiled!'],
}

export function FakeLoadApp() {
  const [preset, setPreset] = useState('OS')
  ...
  function start() {
    setProgress(0); setStep(''); setRunning(true); setDone(false)
    const steps = LOADING_STEPS[preset]
    let i = 0
    const interval = setInterval(() => {
      if (i >= steps.length) { clearInterval(interval); setRunning(false); setDone(true); return }
      setStep(steps[i]); setProgress(Math.round(((i + 1) / steps.length) * 100)); i++
    }, 600)
  }

  return (
    <div className="app-body" style={{ padding: '20px 24px', textAlign: 'center' }}>
      <p className="app-label cyan" style={{ textAlign: 'left' }}>// load.app — Fake Loading Screen</p>
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: 24, flexWrap: 'wrap' }}>
        {Object.keys(LOADING_STEPS).map(p => (
          <button key={p} onClick={() => { setPreset(p); setProgress(0); setStep(''); setDone(false) }}
            style={{ padding: '4px 14px', background: preset === p ? '#00ff4620' : 'transparent', border: `1px solid ${preset === p ? '#00ff46' : '#333'}`, borderRadius: 4, color: preset === p ? '#00ff46' : '#666', fontFamily: 'JetBrains Mono', fontSize: 11, cursor: 'pointer' }}>
            {p}
          </button>
        ))}
      </div>
      <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: 12, padding: '32px', marginBottom: 20 }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>{done ? '✅' : running ? '⏳' : '💻'}</div>
        <p style={{ color: '#aaa', fontFamily: 'JetBrains Mono', fontSize: 13, marginBottom: 16, minHeight: 20 }}>{step || 'Ready to load...'}</p>
        <div style={{ height: 8, background: '#1a1a1a', borderRadius: 4, overflow: 'hidden', marginBottom: 8 }}>
          <div style={{ height: '100%', width: `${progress}%`, background: done ? '#00ff46' : '#00c8ff', borderRadius: 4, transition: 'width 0.4s' }} />
        </div>
        <p style={{ color: '#555', fontFamily: 'JetBrains Mono', fontSize: 11 }}>{progress}%</p>
      </div>
      <button onClick={start} disabled={running}
        style={{ padding: '10px 28px', background: '#00c8ff20', border: '2px solid #00c8ff', borderRadius: 10, color: '#00c8ff', fontFamily: 'JetBrains Mono', fontSize: 13, cursor: running ? 'wait' : 'pointer' }}>
        {running ? '⏳ Loading...' : done ? '↺ Run Again' : '▶ Start Loading'}
      </button>
    </div>
  )
}
...
const ADVICE = [
  ...
  'Add more console.log statements until it works.',
  'The bug is definitely in someone else\'s code.',
  'Close all your tabs and open them again.',
  'The real bug was the friends we made along the way.',
  'Just comment out the broken code. Ship it.',
  'Sleep on it. Your brain debugs during REM.',
  'Submit a Stack Overflow question titled "urgent please help".',
  'git blame and email that person immediately.',
  'Rewrite it in Rust.',
  'Have you considered switching careers?',
  'The semicolon. It\'s always the semicolon.',
  'Your IDE color theme is wrong. Change it. This will help.',
  'Delete node_modules and npm install. Trust the process.',
  'The bug only appears in production because production has feelings.',
]

export function FixBugApp() {
  ...
  function getAdvice() {
    ...
    setTimeout(() => {
      ...
      setAdvice(prev => [a, ...prev.slice(0, 4)]); setSpinning(false)
    }, 400)
  }

  return (
    <div className="app-body" style={{ padding: '20px 24px', textAlign: 'center' }}>
      <p className="app-label cyan" style={{ textAlign: 'left' }}>// fixbug.app — Bug Fix Advisor</p>
      <div style={{ fontSize: 72, margin: '16px 0 8px' }}>🐛</div>
      <p style={{ color: '#d0d0d0', fontFamily: 'JetBrains Mono', fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Bug detected in your code</p>
      <p style={{ color: '#555', fontFamily: 'JetBrains Mono', fontSize: 11, marginBottom: 24 }}>Our expert AI has analyzed the situation.</p>
      <button onClick={getAdvice} disabled={spinning}
        style={{ padding: '10px 28px', background: '#ff505020', border: '2px solid #ff5050', borderRadius: 10, color: '#ff5050', fontFamily: 'JetBrains Mono', fontSize: 13, cursor: spinning ? 'wait' : 'pointer', marginBottom: 20 }}>
        {spinning ? '🔍 Analyzing...' : '🔧 Fix My Bug'}
      </button>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, textAlign: 'left' }}>
        {advice.map((a, i) => (
          <div key={i} style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: 8, padding: '12px 16px', opacity: 1 - i * 0.15 }}>
            <p style={{ color: i === 0 ? '#ff5050' : '#555', fontFamily: 'JetBrains Mono', fontSize: i === 0 ? 13 : 11, margin: 0, fontWeight: i === 0 ? 700 : 400 }}>
              {i === 0 ? '→ ' : ''}{a}
            </p>
          </div>
        ))}
